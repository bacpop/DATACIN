...
impl<'a> SplitKmer<'a> {
    ...
    fn build(
        ...
        is_reads: bool,
        rc: bool,
    ) -> Option<(u128, u128, u8, Option<NtHashIterator>)> {
        if *idx + k >= seq_len {
            return None;
        }
        let mut upper = 0;
        ...
        while i < k {
                    ...
                    || Self::valid_qual(i + *idx, qual, min_qual))
            {
                ...
                match i.cmp(&middle_idx) {
                    ...
                    Ordering::Less => {
                        upper <<= 2;
                        upper |= (next_base as u128) << (middle_idx * 2);
                    }
                    Ordering::Equal => {
    ...
    fn update_rc(&mut self) {
        self.rc_upper = rev_comp(self.lower, self.k - 1) & self.upper_mask;
        self.rc_middle_base = rc_base(self.middle_base);
        self.rc_lower = rev_comp(self.upper, self.k - 1) & self.lower_mask;
    }
    ...
    fn roll_fwd(&mut self) -> bool {
                ...
                && !Self::valid_qual(self.index, self.qual, self.min_qual))
        {
            ...
            if let Some(ref mut roll_hash) = self.hash_gen {
                ...
                roll_hash.roll_fwd(old_base as u8, new_base);
            }
            ...
            if self.rc {
                ...
                self.rc_upper = (self.rc_upper >> 2
                    | (rc_base(new_base) as u128) << (2 * ((half_k * 2) - 1)))
                    & self.upper_mask;
            }
            success = true;
        }
    ...
    pub fn get_curr_kmer(&self) -> (u128, u8, bool) {
    ...
    pub fn get_next_kmer(&mut self) -> Option<(u128, u8, bool)> {
